...
void CTaskBarDlg::ShowInfo(CDC* pDC)
{
    ...
    int base_line_height = GetBaseLineHeight();
    int side_item_height = GetSideItemHeight();
    int vertical_margin = DPI(theApp.m_taskbar_data.vertical_margin);
        ...
        if (IsTasksbarOnTopOrBottom())
        {
            ...
            else //非水平排列时，每两个一组显示
            {
                ...
                if (index % 2 == 1)
                {
                    ...
                    if (index > 0)
                        item_rect_up.MoveToXY(item_rect.right + DPI(theApp.m_taskbar_data.item_space), -vertical_margin);
                    item_rect.left = item_rect_up.left;
                    item_rect.top = base_line_height;
                    //确定窗口大小
...
void CTaskBarDlg::DrawDisplayItem(IDrawCommon& drawer, DisplayItem type, CRect rect, int label_width, bool vertical)
{
        ...
        || type == TDI_UP || type == TDI_DOWN || type == TDI_TOTAL_SPEED/* ||type==TDI_CPU_FREQ*/)
    {
        ...
        {
        case TDI_CPU:
            figure_value = monitor_data.cpu_usage;
            break;
        case TDI_MEMORY:
            figure_value = monitor_data.memory_usage;
            break;
        case TDI_GPU_USAGE:
            figure_value = monitor_data.gpu_usage;
            break;
        case TDI_CPU_TEMP:
            figure_value = static_cast<int>(monitor_data.cpu_temperature);
            break;
        case TDI_GPU_TEMP:
            figure_value = static_cast<int>(monitor_data.gpu_temperature);
            break;
        case TDI_HDD_TEMP:
            figure_value = static_cast<int>(monitor_data.hdd_temperature);
            break;
        case TDI_MAIN_BOARD_TEMP:
            figure_value = static_cast<int>(monitor_data.main_board_temperature);
            break;
        case TDI_HDD_USAGE:
            figure_value = monitor_data.hdd_usage;
            break;
        //case TDI_CPU_FREQ:
        //    figure_value = theApp.m_cpu_freq;
        //    break;
        case TDI_UP:
            figure_value = CalculateNetspeedPercent(monitor_data.out_speed);
            break;
        case TDI_DOWN:
            figure_value = CalculateNetspeedPercent(monitor_data.in_speed);
            break;
        case TDI_TOTAL_SPEED:
            figure_value = CalculateNetspeedPercent(monitor_data.in_speed + monitor_data.out_speed);
            break;
        default:
            break;
        }
    ...
    if (type == TDI_UP || type == TDI_DOWN || type == TDI_TOTAL_SPEED)
    {
        ...
        if (theApp.m_taskbar_data.hide_unit && theApp.m_taskbar_data.speed_unit != SpeedUnit::AUTO)
            ...
            format_str = _T("%s/s");
        CString str_in_speed = CCommon::DataSizeToString(monitor_data.in_speed, theApp.m_taskbar_data);
        CString str_out_speed = CCommon::DataSizeToString(monitor_data.out_speed, theApp.m_taskbar_data);
        CString str_total_speed = CCommon::DataSizeToString(monitor_data.in_speed + monitor_data.out_speed, theApp.m_taskbar_data);
        //if (theApp.m_taskbar_data.swap_up_down)
    ...
    if (type == TDI_MEMORY && (theApp.m_taskbar_data.memory_display == MemoryDisplay::MEMORY_USED || theApp.m_taskbar_data.memory_display == MemoryDisplay::MEMORY_AVAILABLE))
    {
        if (theApp.m_taskbar_data.memory_display == MemoryDisplay::MEMORY_USED)
            str_value = CCommon::DataSizeToString(static_cast<unsigned long long>(monitor_data.used_memory) * 1024, theApp.m_taskbar_data.separate_value_unit_with_space);
        else
            str_value = CCommon::DataSizeToString((static_cast<unsigned long long>(monitor_data.total_memory) - static_cast<unsigned long long>(monitor_data.used_memory)) * 1024, theApp.m_taskbar_data.separate_value_unit_with_space);
    }
    //绘制CPU或内存利用率
    else if (type == TDI_CPU || type == TDI_MEMORY || type == TDI_GPU_USAGE || type == TDI_HDD_USAGE)
    {
        ...
        case TDI_MEMORY:
            ...
            break;
        case TDI_GPU_USAGE:
            usage = monitor_data.gpu_usage;
            break;
        case TDI_HDD_USAGE:
            usage = monitor_data.hdd_usage;
            break;
        default:
            break;
        }
    ...
    else if (type == TDI_CPU_TEMP || type == TDI_GPU_TEMP || type == TDI_HDD_TEMP || type == TDI_MAIN_BOARD_TEMP)
    {
        ...
        {
        case TDI_CPU_TEMP:
            temperature = static_cast<int>(monitor_data.cpu_temperature);
            break;
        case TDI_GPU_TEMP:
            temperature = static_cast<int>(monitor_data.gpu_temperature);
            break;
        case TDI_HDD_TEMP:
            temperature = static_cast<int>(monitor_data.hdd_temperature);
            break;
        case TDI_MAIN_BOARD_TEMP:
            temperature = static_cast<int>(monitor_data.main_board_temperature);
            break;
        default:
            break;
        }
        ...
        str_value = CCommon::FreqToString(monitor_data.cpu_freq, theApp.m_taskbar_data);
    }
    else if (type == TDI_TODAY_TRAFFIC)
    {
        str_value = CCommon::KBytesToString((monitor_data.today_up_traffic + monitor_data.today_down_traffic) / 1024u);
    }
...
CString CTaskBarDlg::GetMouseTipsInfo()
{
    ...
    if (!IsShowCpuMemory())
    {
        temp.Format(_T("\r\n%s: %s/%s"),
            ...
            CCommon::KBytesToString(monitor_data.used_memory), CCommon::KBytesToString(monitor_data.total_memory));
    }
    ...
    if (!IsItemShow(TDI_CPU_FREQ) && monitor_data.cpu_freq > 0)
    {
        temp.Format(_T("\r\n%s: %s"), CCommon::LoadText(IDS_CPU_FREQ), CCommon::FreqToString(monitor_data.cpu_freq, theApp.m_taskbar_data));
        tip_info += temp;
    }
    ...
    {
        if (!IsItemShow(TDI_GPU_USAGE) && monitor_data.gpu_usage >= 0)
        {
            temp.Format(_T("\r\n%s: %d %%"), CCommon::LoadText(IDS_GPU_USAGE), monitor_data.gpu_usage);
            tip_info += temp;
        }
        if (!IsItemShow(TDI_CPU_TEMP) && monitor_data.cpu_temperature > 0)
        {
            temp.Format(_T("\r\n%s: %s"), CCommon::LoadText(IDS_CPU_TEMPERATURE), CCommon::TemperatureToString(monitor_data.cpu_temperature, theApp.m_taskbar_data));
            tip_info += temp;
        }
        if (!IsItemShow(TDI_GPU_TEMP) && monitor_data.gpu_temperature > 0)
        {
            temp.Format(_T("\r\n%s: %s"), CCommon::LoadText(IDS_GPU_TEMPERATURE), CCommon::TemperatureToString(monitor_data.gpu_temperature, theApp.m_taskbar_data));
            tip_info += temp;
        }
        if (!IsItemShow(TDI_HDD_TEMP) && monitor_data.hdd_temperature > 0)
        {
            temp.Format(_T("\r\n%s: %s"), CCommon::LoadText(IDS_HDD_TEMPERATURE), CCommon::TemperatureToString(monitor_data.hdd_temperature, theApp.m_taskbar_data));
            tip_info += temp;
        }
        if (!IsItemShow(TDI_MAIN_BOARD_TEMP) && monitor_data.main_board_temperature > 0)
        {
            temp.Format(_T("\r\n%s: %s"), CCommon::LoadText(IDS_MAINBOARD_TEMPERATURE), CCommon::TemperatureToString(monitor_data.main_board_temperature, theApp.m_taskbar_data));
            tip_info += temp;
        }
        if (!IsItemShow(TDI_HDD_USAGE) && monitor_data.hdd_usage >= 0)
        {
...
void CTaskBarDlg::SetTextFont()
{
    ...
    if (m_pDC != nullptr)
    {
        CFont* old_font = m_pDC->SelectObject(&m_font);
        TEXTMETRIC text_metric{};
        if (m_pDC->GetTextMetrics(&text_metric))
        {
            m_text_line_height = text_metric.tmHeight + text_metric.tmExternalLeading;
        }
        if (old_font != nullptr)
            m_pDC->SelectObject(old_font);
    }
    if (m_text_line_height <= 0)
        m_text_line_height = DPI(16);
}
...
int CTaskBarDlg::GetBaseLineHeight() const
{
    int base_height = (m_text_line_height > 0 ? m_text_line_height : DPI(16));
    int min_height = TASKBAR_WND_HEIGHT / 2;
    if (base_height < min_height)
        base_height = min_height;
    return base_height;
}

int CTaskBarDlg::GetSingleLineHeight() const
{
    int base_height = (m_text_line_height > 0 ? m_text_line_height : DPI(16));
    int min_height = TASKBAR_WND_HEIGHT * 2 / 3;
    int height = base_height + DPI(2);
    if (height < min_height)
        height = min_height;
    return height;
}

int CTaskBarDlg::GetSideItemHeight() const
{
    int base_height = (m_text_line_height > 0 ? m_text_line_height : DPI(16));
    int min_height = TASKBAR_WND_HEIGHT / 2;
    int height = base_height + DPI(2);
...
void CTaskBarDlg::CalculateWindowSize()
{
    ...
    {
        //宽度为总内存的宽度
        str = CCommon::DataSizeToString(static_cast<unsigned long long>(monitor_data.total_memory) * 1024, theApp.m_taskbar_data.separate_value_unit_with_space);
    ...
    if (IsTasksbarOnTopOrBottom())
    {
        if (!horizontal_arrange)
            m_window_height = GetBaseLineHeight() * 2;
        else
            m_window_height = GetSingleLineHeight();
    }
    else
    {
        m_window_height = GetSideItemHeight() * item_count;
...
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    // wParam=1 表示任务栏仍存在，通常为用户主动关闭
    bool taskbar_window_valid = (m_hTaskbar != nullptr && ::IsWindow(m_hTaskbar));
    ::SendMessage(theApp.m_pMainWnd->GetSafeHwnd(), WM_TASKBAR_WND_CLOSED, taskbar_window_valid ? 1 : 0, 0);
