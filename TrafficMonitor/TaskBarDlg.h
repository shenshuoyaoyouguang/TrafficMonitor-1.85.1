...
protected:
    ...
    int m_text_line_height{};      //任务栏窗口字体的实际行高（像素）
    ...
    int GetBaseLineHeight() const;        //双行布局时单行的基础高度
    int GetSingleLineHeight() const;      //单行布局时窗口高度
    int GetSideItemHeight() const;        //任务栏在左右时每项高度
