...
void CSkinFile::DrawItemsInfo(IDrawCommon& drawer, Layout& layout, CFont& font)
{
    ...
    //上传/下载
    CString in_speed = CCommon::DataSizeToString(monitor_data.in_speed, theApp.m_main_wnd_data);
    CString out_speed = CCommon::DataSizeToString(monitor_data.out_speed, theApp.m_main_wnd_data);
    CString total_speed = CCommon::DataSizeToString(monitor_data.in_speed + monitor_data.out_speed, theApp.m_main_wnd_data);
    ...
    //CPU/内存/显卡利用率
    map_str[TDI_CPU].value = CCommon::UsageToString(monitor_data.cpu_usage, theApp.m_main_wnd_data);

    map_str[TDI_CPU_FREQ].value = CCommon::FreqToString(monitor_data.cpu_freq, theApp.m_main_wnd_data);
    CString str_memory_value;
    if (theApp.m_main_wnd_data.memory_display == MemoryDisplay::MEMORY_USED)
        str_memory_value = CCommon::DataSizeToString(static_cast<unsigned long long>(monitor_data.used_memory) * 1024, theApp.m_main_wnd_data.separate_value_unit_with_space);
    else if (theApp.m_main_wnd_data.memory_display == MemoryDisplay::MEMORY_AVAILABLE)
        str_memory_value = CCommon::DataSizeToString((static_cast<unsigned long long>(monitor_data.total_memory) - static_cast<unsigned long long>(monitor_data.used_memory)) * 1024, theApp.m_main_wnd_data.separate_value_unit_with_space);
    else
        str_memory_value = CCommon::UsageToString(monitor_data.memory_usage, theApp.m_main_wnd_data);
    map_str[TDI_MEMORY].value = str_memory_value;
    map_str[TDI_GPU_USAGE].value = CCommon::UsageToString(monitor_data.gpu_usage, theApp.m_main_wnd_data);
    map_str[TDI_HDD_USAGE].value = CCommon::UsageToString(monitor_data.hdd_usage, theApp.m_main_wnd_data);
    ...
    getTemperatureStr(TDI_CPU_TEMP, monitor_data.cpu_temperature);
    getTemperatureStr(TDI_GPU_TEMP, monitor_data.gpu_temperature);
    getTemperatureStr(TDI_HDD_TEMP, monitor_data.hdd_temperature);
    getTemperatureStr(TDI_MAIN_BOARD_TEMP, monitor_data.main_board_temperature);

    //总流量
    CString str_traffic = CCommon::KBytesToString((monitor_data.today_up_traffic + monitor_data.today_down_traffic) / 1024u);
