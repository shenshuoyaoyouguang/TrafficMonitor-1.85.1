...
UINT CNetworkInfoDlg::GetInternetIPThreadFunc(LPVOID lpParam)
{
    ...
    HWND target_hwnd = (p_instance != nullptr) ? p_instance->GetSafeHwnd() : NULL;
    wstring ip_address, ip_location;
    auto post_update = [target_hwnd](int row, const CString& text)
        {
            if (!IsWindow(target_hwnd))
                return false;
            auto* payload = new InternetIpUpdate();
            payload->row = row;
            payload->text = text.GetString();
            if (!::PostMessage(target_hwnd, WM_INET_IP_UPDATED, 0, reinterpret_cast<LPARAM>(payload)))
            {
                delete payload;
                return false;
            }
    ...
    if (!ip_address.empty())
    {
        ...
        if (!post_update(14, info))
            return 0;
    }
    else
    {
        if (!post_update(14, CCommon::LoadText(IDS_GET_FAILED)))
            return 0;
    }
    ...
    if (!ip_address.empty())
    {
        ...
        if (ip_location.empty())
            ...
            info.Format(_T("%s (%s)"), ip_address.c_str(), ip_location.c_str());
        if (!post_update(15, info))
            return 0;
    }
    else
    {
        if (!post_update(15, CCommon::LoadText(IDS_GET_FAILED)))
            return 0;
    }
...
BEGIN_MESSAGE_MAP(CNetworkInfoDlg, CBaseDialog)
    ...
    ON_MESSAGE(WM_INET_IP_UPDATED, &CNetworkInfoDlg::OnInetIpUpdated)
END_MESSAGE_MAP()
...
LRESULT CNetworkInfoDlg::OnInetIpUpdated(WPARAM wParam, LPARAM lParam)
{
    UNREFERENCED_PARAMETER(wParam);
    auto* payload = reinterpret_cast<InternetIpUpdate*>(lParam);
    if (payload == nullptr)
        return 0;
    m_info_list.SetItemText(payload->row, 1, payload->text.c_str());
    delete payload;
    payload = nullptr;
    m_ip_update_count++;
    if (m_ip_update_count >= 2)
        m_ip_acquired = true;
    return 0;
}
