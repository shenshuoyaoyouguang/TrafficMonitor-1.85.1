...
namespace
{
    std::vector<int> SplitVersionNumbers(const std::wstring& version)
    {
        std::vector<int> parts;
        size_t start = 0;
        while (start <= version.length())
        {
            size_t dot_pos = version.find(L'.', start);
            std::wstring token = (dot_pos == std::wstring::npos) ? version.substr(start) : version.substr(start, dot_pos - start);
    ...
    int CompareVersionNumbers(const std::wstring& left, const std::wstring& right)
    {
        auto left_parts = SplitVersionNumbers(left);
        auto right_parts = SplitVersionNumbers(right);
        size_t size = std::max(left_parts.size(), right_parts.size());
        left_parts.resize(size);
        right_parts.resize(size);
...
void CTrafficMonitorApp::CheckUpdate(bool message)
{
    ...
    if (CompareVersionNumbers(version, VERSION) > 0)      //如果服务器上的版本大于本地版本
    {
        CString info;
        //根据语言设置选择对应语言版本的更新内容
...
MonitorDataSnapshot CTrafficMonitorApp::GetMonitorDataSnapshot()
{
    CSingleLock lock(&m_monitor_data_critical, TRUE);
    return m_monitor_data_snapshot;
}

void CTrafficMonitorApp::UpdateMonitorDataSnapshot()
{
    CSingleLock lock(&m_monitor_data_critical, TRUE);
    m_monitor_data_snapshot.in_speed = m_in_speed;
    m_monitor_data_snapshot.out_speed = m_out_speed;
    m_monitor_data_snapshot.cpu_usage = m_cpu_usage;
    m_monitor_data_snapshot.memory_usage = m_memory_usage;
    m_monitor_data_snapshot.used_memory = m_used_memory;
    m_monitor_data_snapshot.total_memory = m_total_memory;
    m_monitor_data_snapshot.cpu_temperature = m_cpu_temperature;
    m_monitor_data_snapshot.cpu_freq = m_cpu_freq;
    m_monitor_data_snapshot.gpu_temperature = m_gpu_temperature;
    m_monitor_data_snapshot.hdd_temperature = m_hdd_temperature;
    m_monitor_data_snapshot.main_board_temperature = m_main_board_temperature;
    m_monitor_data_snapshot.gpu_usage = m_gpu_usage;
    m_monitor_data_snapshot.hdd_usage = m_hdd_usage;
    m_monitor_data_snapshot.today_up_traffic = m_today_up_traffic;
    m_monitor_data_snapshot.today_down_traffic = m_today_down_traffic;
}
